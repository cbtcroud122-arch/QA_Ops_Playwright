import {test,expect} from '@playwright/test';
  ...
  let webContext; //declare at global level so we can use this in all tests 
    ...
    //Login and stort the session state 
    ...
    //this will store the session details in the file -it should be Browser level so Context.StorageState -- We have to Store it path (root directory) with name State.json
    await context.storageState({path :'state.json'})
    //Creating new browser context and then injecting it to the browser session so it will ahve all session details 
    webContext = await browser.newContext({storageState:'state.json'});

});

test('client App E2E testing with session stoge', async ()=>
{

    const page = await webContext.newPage();
    let ProductName = "ADIDAS ORIGINAL";
    let Selectcountry = " India";
     // here we have to create a new page with WebContext so it will use all session details 
...
await expect(page.locator('div label')).toHaveText("cbtcroud122@gmail.com");
...
await expect(page.locator(".user__name [type='text']").first()).toHaveText("cbtcroud122@gmail.com");
